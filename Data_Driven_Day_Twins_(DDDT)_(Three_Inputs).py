...
input2 = pd.read_excel(file_path, sheet_name="Input2").values.flatten()
input3 = pd.read_excel(file_path, sheet_name="Input3").values.flatten()
...
input_test2 = pd.read_excel(file_path, sheet_name="Input_Test2").values.flatten()
input_test3 = pd.read_excel(file_path, sheet_name="Input_Test3").values.flatten()
...
def build_model():
    input_layer = Input(shape=(24, 3, 1), name='input')
...
def iterative_predict(model, input_test1, input_test2, input_test3, output_test, window_size=24):
    predictions = []
    current_window = np.stack([input_test1[:window_size],
                              input_test2[:window_size],
                              input_test3[:window_size]], axis=-1)
    ...
    for i in tqdm(range(max_steps), desc="Predicting"):
        if current_window.shape != (1, 24, 3, 1):
            current_window = current_window.reshape(1, 24, 3, 1)
        ...
        if i < max_steps - 1:
            ...
            new_input3 = input_test3[new_idx]

            current_window = np.roll(current_window, shift=-1, axis=1)
            current_window[0, -1, :, 0] = [new_input1, new_input2, new_input3]

    return np.array(predictions)

# Make predictions
predictions_test = iterative_predict(model, input_test1, input_test2, input_test3, output_test)
